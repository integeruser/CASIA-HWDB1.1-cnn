...
```
~ ➤ pip2 install --user git+git://github.com/Theano/Theano.git
~ ➤ pip2 install --user h5py keras pillow scikit-image
```
...
Convert the downloaded data set into the HDF5 binary data format (~20 min):
```
...
Extract from the data set 200 classes of characters (~10 min):
```
...
(Optional) Generate a report of some classifications (~20 sec):
```
